...
export default function Footer() {
	...
	useEffect(() => {
		...
		if (emailInputRef.current) {
			...
			emailMaskRef.current.on('accept', () => {
				const emailValue = emailMaskRef.current.value
				// Обрезаем email до 50 символов
				const trimmedEmail =
					emailValue.length > 50 ? emailValue.substring(0, 50) : emailValue
	...
	const onSubmit = async data => {
		...
		// Обрезка name и email до 50 символов
		const trimmedName =
			data.name.length > 50 ? data.name.substring(0, 50) : data.name
		const trimmedEmail =
			data.email.length > 50 ? data.email.substring(0, 50) : data.email
	...
	// Обработчик изменения поля name с обрезкой
	const handleNameChange = e => {
		const value = e.target.value
		const trimmedValue = value.length > 50 ? value.substring(0, 50) : value
		setValue('name', trimmedValue, { shouldValidate: true })
	}

	// Обработчик изменения поля email с обрезкой
